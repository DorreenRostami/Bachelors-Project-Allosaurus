...
class PhoneDecoder:
    ...
    def compute(self, logits, lang_id=None, topk=1, emit=1.0, timestamp=False):
        ...
        # sourceFile = open('FARSDAT-parent-c100\\sample-resampled-logits.txt', 'w+')
        # print("logits (row,vol) before mask: ", logits.shape, file = sourceFile)
        # for i in range(logits.shape[0]):
        #     for j in range(logits.shape[1]):
        #         print(logits[i][j], ", ", file = sourceFile, end='')
        #     print("\n", file = sourceFile)

        logits = mask.mask_logits(logits)

        # print("\n\n\n\n\n\n\n\n\n\nlogits (row,vol) after mask: ", logits.shape, file = sourceFile)
        # for i in range(logits.shape[0]):
        #     for j in range(logits.shape[1]):
        #         print(logits[i][j], ", ", file = sourceFile, end='')
        #     print("\n", file = sourceFile)
